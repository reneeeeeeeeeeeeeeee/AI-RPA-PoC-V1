# ERP Assistant Backend
# FastAPI Server - Chat directly to CogAgent + Excel workflow
...
app = FastAPI(title="ERP Assistant")
...
# Per WebSocket session: dedicated CogAgent history
...
async def handle_chat(text: str, file_path: Optional[str], ws: WebSocket, session_id: str):
    ...
    # ── Pending context (cost-centre follow-up) ────────────────────────
    ...
    # ── Orchestrator splits prompt into steps, CogAgent executes ───────
...
async def _orchestrated_execute(prompt: str, ws: WebSocket, session_id: str):
    """Splits a prompt into individual steps and executes each one separately."""
    import re

    # Extract steps from prompt
    # Delimiters: line break, " then ", " and then ", or sentence end before a new verb
...
def _split_into_steps(prompt: str) -> list:
    """Splits a prompt into individual action steps."""
    import re
...
async def _direct_cogagent(task: str, ws: WebSocket, session_id: str):
    """Sends a task directly to CogAgent, executes the action, and returns the result."""
    history = session_history.get(session_id, [])
...
def _cogagent_step(task: str, history: list) -> dict:
    """Runs in a thread: screenshot -> OCR/CogAgent -> execute action."""
    ...
    def ocr_click(target, img=None, confidence=30):
        """Finds text via OCR and clicks it. Returns (ax, ay) or None."""
        hits = executor._find_text_on_screen(target, img)
        if not hits:
            print(f"[Step] OCR: '{target}' not found")
            return None
        ox, oy, conf = hits[0]
        if conf < confidence:
            print(f"[Step] OCR: '{target}' confidence too low ({conf})")
            return None
    ...
    if type_match:
        # Text after "field:" or after the last colon
        if colon_match:
    ...
    if dropdown_match:
        ...
        if dropdown_field:
            ...
            # Attempt 1: OCR with lower contrast threshold (psm 6)
            ...
            found_y = None
            # Ignore top 15% of the image (URL bar, browser tabs)
            ...
            for idx2, t in enumerate(data["text"]):
                ...
                if (first_word.lower() in t.lower()
                    ...
                    print(f"[Step] Dropdown label '{t}' at y={found_y} (scaled, min_y={min_y})")
                    break

            if found_y:
                # Dropdown box is to the right of the label, approx. at x=42% of window width
                ax = int(0.42 * sw * ww / sw) + off[0]
                ay = int(found_y * wh / sh) + off[1]
            else:
                # Fallback: known relative position for "Main currency" (~42% left, ~27% top)
            ...
            _pag.click(ax, ay)
            print(f"[Step] Dropdown clicked at ({ax},{ay})")
            _t.sleep(1.0)  # Wait until dropdown is open

        # Type into search field
        _pag.typewrite(choice, interval=0.06)
        _t.sleep(0.8)

        # Find and click the option via OCR
    ...
    if scroll_match:
        ...
        # Click Save if mentioned in the task
