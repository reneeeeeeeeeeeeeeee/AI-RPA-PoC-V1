...
# Path to transformers modeling_utils.py
import transformers
...
class ERPExecutor:
    ...
    def _patch_config(self):
        """Adds a max_length property to ChatGLMConfig that delegates to seq_length.

        CogAgent uses seq_length internally, but newer transformers versions read
        max_length directly on the config object. Without this, generation raises
        AttributeError.
        """
    ...
    def _patch_modeling(self):
        """Makes ChatGLMForConditionalGeneration compatible with transformers' generation API.

        Adds GenerationMixin to the class hierarchy and initialises tied-weight
        attributes with the types transformers expects. Without this the model
    ...
    def _patch_transformers_disk(self):
        """Patches transformers/modeling_utils.py on disk to accept list-typed tied-weight keys.

        CogAgent stores all_tied_weights_keys as a list; transformers assumes a dict
        and calls .keys() on it. This replaces the three affected call sites in
        modeling_utils.py with list-safe equivalents.
        """
        if not TRANSFORMERS_UTILS.exists():
            print(f"[CogAgent] modeling_utils.py not found: {TRANSFORMERS_UTILS}")
            return

        text = TRANSFORMERS_UTILS.read_text(encoding="utf-8")

        if "PATCH_TIED_LIST" in text:
            print("[CogAgent] Patch 3: transformers already patched.")
            return
        ...
        for old, new in replacements:
            if old in text:
                ...
                changed = True
                print(f"[CogAgent] Patch 3: replaced: {old[:60]}...")

        if changed:
            TRANSFORMERS_UTILS.write_text(text, encoding="utf-8")
            print("[CogAgent] Patch 3: transformers/modeling_utils.py patched.")
        else:
            print("[CogAgent] Patch 3: No matching locations found - possibly a different transformers version.")
            # Show relevant lines for diagnostics
            for i, line in enumerate(text.splitlines()):
                if "all_tied_weights_keys" in line:
                    print(f"  Line {i+1}: {line.strip()}")

    def _patch_model_runtime(self):
        """Applies compatibility fixes to the loaded model instance at runtime.

        CogAgent's model class is missing several attributes and methods that
        transformers' generation pipeline requires. These cannot be injected via
        source-file patches because they depend on the instantiated objects.
        """
        import torch
        ...
        if GenerationMixin not in cls.__mro__:
            ...
            print("[CogAgent] GenerationMixin injected.")
        ...
        print("[CogAgent] generation_config set.")
        ...
        if not hasattr(self.model, "_extract_past_from_model_output"):
            ...
            print("[CogAgent] _extract_past_from_model_output set.")
    ...
    def _load_cogagent(self):
        try:
            ...
            print(f"[CogAgent] Loading model from {COGAGENT_PATH} ...")

            self._patch_config()
            self._patch_modeling()
            self._patch_transformers_disk()

            # Reimport transformers so patches take effect
            import importlib
            import transformers.modeling_utils
            importlib.reload(transformers.modeling_utils)
            print("[CogAgent] transformers.modeling_utils reloaded.")
            ...
            if not torch.cuda.is_available():
                print("[CogAgent] CUDA not available.")
                return
            ...
            print("[CogAgent] Model loaded successfully.")

        except Exception as e:
            print(f"[CogAgent] WARNING: Model not loaded: {e}")
            import traceback
            traceback.print_exc()
    ...
    def _focus_ie(self):
        """Brings the ERP window (IE, Edge or Chrome with ERP) to the foreground."""
        import pygetwindow as gw, time

        # All window keywords that may contain the ERP
        keywords = [
        ...
        # Show all open windows (on first call only)
        if not hasattr(self, "_windows_logged"):
            self._windows_logged = True
            all_titles = [w.title for w in gw.getAllWindows() if w.title]
            print(f"[CogAgent] Open windows: {all_titles}")

        wins = [w for w in gw.getAllWindows()
                if any(k.lower() in w.title.lower() for k in keywords)]

        if not wins:
            # No matching window found - print all windows
            print("[CogAgent] No ERP window found!")
            print(f"[CogAgent] Available: {[w.title for w in gw.getAllWindows() if w.title]}")
            return False

        try:
            w = wins[0]
            print(f"[CogAgent] Focusing: {w.title}")
            w.restore()
            w.activate()
            time.sleep(0.5)
            return True
        except Exception as e:
            print(f"[CogAgent] Focus failed: {e}")
            return False

    def _screenshot(self):
        """Screenshot from Edge/IE window only - not CMD or other windows."""
        focused = self._focus_ie()
        import time
        time.sleep(0.8)

        # Try screenshot from Edge window only
        try:
            import pygetwindow as gw
            ...
            if wins:
                w = wins[0]
                # Maximize window and wait until it is fully maximized
                try:
                    w.maximize()
                    time.sleep(0.8)
                    w.activate()
                ...
                iw, ih = img.size
                print(f"[CogAgent] Window: {iw}x{ih} @ ({left},{top})")

                # Scale to 1920x1080 for consistent results
            ...
            print(f"[CogAgent] Window screenshot failed: {e}")

        # Fallback: full screen scaled
    ...
    def _ask_cogagent(self, img, task: str, history: list) -> dict:
        if self.model is None:
            return {"action": "error", "thought": "Model not loaded"}

        import torch

        # History string according to CogAgent docs
        history_str = "\nHistory steps: "
        for idx, h in enumerate(history):
            history_str += f"\n{idx}. {h.get('operation','')}"

        # CogAgent supports different format strings:
        # - "Action-Operation-Sensitive" -> sometimes returns no coordinates
        # - "Grounded-Operation" -> enforces CLICK(box=[[...]]) format
        platform_str = "(Platform: WIN)\n"
        format_str   = "(Answer in Grounded-Operation format.)\n"
        query = f"Task: {task}{history_str}\n{platform_str}{format_str}"

        # Correct format according to tokenization_chatglm.py:
        # A dict with role + image + content (all together)
        # content MUST be a string, image is PIL.Image
        conversation = [
            {
                "role":    "user",
        ...
        try:
            ...
            # Recalculate attention mask AFTER tokenizing
            # Tokenizer returns mask for text (36 tokens),
            # but input_ids contains image patches (1600 extra tokens)
            # -> mask must match full length of input_ids
            ...
            print(f"[CogAgent] Tokenizer error: {e}")
            import traceback; traceback.print_exc()
            return {"action": "error", "thought": str(e)}

        try:
            # Set ChatGLM4 EOS tokens correctly
            ...
            # Manual greedy decode instead of model.generate()
            # generate() loses images during prepare_inputs_for_generation
            ...
            print(f"[CogAgent] Response: {raw[:300]}")
        except Exception as e:
            print(f"[CogAgent] Error: {e}")
            import traceback; traceback.print_exc()
            return {"action": "error", "thought": str(e)}
    ...
    def _parse_cogagent_output(self, raw: str, task: str = "") -> dict:
        ...
        # CogAgent returns coordinates in 0-1000 space relative to the scaled image
        # _scaled_size: size of the image passed to CogAgent (e.g. 1920x1080)
        # _window_size: original window size for back-scaling
        # _window_offset: window position on screen
        ...
        # First: 0-1000 -> scaled image (e.g. 1920x1080)
        # Then: scaled image -> original window
        sw, sh = scaled_w, scaled_h

        # OCR first for "click on TEXT" tasks - more reliable than CogAgent coordinates
        import re as _re2
        ...
        if m_task:
            ...
            if ocr_hits:
                ...
                # OCR coordinates are in scaled image space (e.g. 1920x1080)
                # Return directly as absolute window coordinates
                # _execute_action must NOT scale again
        ...
        # CLICK - only if coordinates are plausible (> 10px from edge)
        MIN_COORD = 20  # Coordinates below this value are usually errors
        ...
        if m:
            ...
            if cx > MIN_COORD and cy > MIN_COORD:
                ...
                return result
            print(f"[CogAgent] CLICK(4pts) invalid: ({cx},{cy}) - too close to edge")
        ...
        if m:
            ...
            if cx > MIN_COORD and cy > MIN_COORD:
                ...
                return result
            print(f"[CogAgent] CLICK(2pts) invalid: ({cx},{cy}) - too close to edge")

        # Single coordinate or invalid coords -> fall through to OCR
        ...
        # OCR fallback: search for target text from task or CogAgent response
        search_targets = []
        # 1. From the original task
        import re as _re3
        ...
        print(f"[CogAgent] Unknown format: {raw[:100]}")
        return result

    def _find_text_on_screen(self, search_text: str, img=None) -> list:
        """Finds text on screen via OCR. Returns list of (x, y) coordinates."""
        try:
            import pytesseract
            if img is None:
    ...
    def _execute_action(self, action: dict):
        ...
        if act in ("click", "ocr_click"):
            ...
            if act == "ocr_click":
                # OCR coordinates are already absolute
                print(f"[CogAgent] OCR click absolute: ({x},{y})")
                pyautogui.click(x, y)
            else:
                # CogAgent coordinates: scaled -> window -> absolute
                scaled_w, scaled_h = getattr(self, "_scaled_size",  pyautogui.size())
                win_w,    win_h    = getattr(self, "_window_size",   pyautogui.size())
                off_x,    off_y    = getattr(self, "_window_offset", (0, 0))
                ...
                abs_x = win_x + off_x
                abs_y = win_y + off_y
                print(f"[CogAgent] Click: scaled({x},{y}) -> window({win_x},{win_y}) + offset({off_x},{off_y}) = absolute({abs_x},{abs_y})")
    ...
    def _open_erp(self, module_path: str):
        """Navigates to an ERP page in Internet Explorer."""
        import time, subprocess
        import pygetwindow as gw

        url = f"{ERP_URL}/{module_path}"

        # Check if IE is already open
        ie_wins = [w for w in gw.getAllWindows()
                   if any(k in w.title for k in ["Internet Explorer", "ERP"])]

        if ie_wins:
            # IE already open - activate window and enter URL via address bar
            try:
                ie_wins[0].restore()
                ie_wins[0].activate()
                time.sleep(0.5)
                ...
                print(f"[CogAgent] IE navigation failed: {e}")

        # Restart IE - try different paths
        ie_binary = shutil.which("iexplore")
        if ie_binary:
            subprocess.Popen([ie_binary, url])
            time.sleep(3)
        else:
            # Fallback: launch via cmd
