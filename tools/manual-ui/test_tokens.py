"""
test_tokens.py - With digit-forcing for complete coordinates
"""
...
print(f"Window: {WIN_W}x{WIN_H} @ ({OFF_X},{OFF_Y})")
...
with torch.no_grad():
    for step in range(60):
        ...
        # Digit-forcing: after comma or colon inside a CLICK box
        ...
        # Colon inside CLICK box -> replace with comma
...
# Extract coordinates and calculate click position
m = re.search(r"CLICK\(box=\[\[\s*(\d+)[^\d]+(\d+)[^\d]+(\d+)[^\d]+(\d+)", raw)
m2 = re.search(r"CLICK\(box=\[\[\s*(\d+)[^\d]+(\d+)", raw)
m1 = re.search(r"CLICK\(box=\[\[\s*(\d+)", raw)

# CogAgent coordinates are relative to 1000x1000 of the input image
# Input image is WIN_W x WIN_H -> scale accordingly
if m:
    x1,y1,x2,y2 = int(m.group(1)),int(m.group(2)),int(m.group(3)),int(m.group(4))
    cx = int((x1+x2)/2 * WIN_W / 1000)
    cy = int((y1+y2)/2 * WIN_H / 1000)
    print(f"\nCLICK 4pts: [{x1},{y1},{x2},{y2}] -> window ({cx},{cy}) -> screen ({cx+OFF_X},{cy+OFF_Y})")
elif m2:
    x1,y1 = int(m2.group(1)),int(m2.group(2))
    cx = int(x1 * WIN_W / 1000)
    cy = int(y1 * WIN_H / 1000)
    print(f"\nCLICK 2pts: [{x1},{y1}] -> window ({cx},{cy}) -> screen ({cx+OFF_X},{cy+OFF_Y})")
elif m1:
    x1 = int(m1.group(1))
    cx = int(x1 * WIN_W / 1000)
    cy = WIN_H // 3
    print(f"\nCLICK 1pt: [{x1}] -> window ({cx},{cy}) -> screen ({cx+OFF_X},{cy+OFF_Y})")
else:
    print("No coordinates found."); sys.exit(0)

abs_x, abs_y = cx + OFF_X, cy + OFF_Y
confirm = input(f"\nClick at absolute ({abs_x},{abs_y})? (y/n): ").strip().lower()
if confirm == "y":
