"""
test_formats.py - Screenshot check with 4K scaling
"""
...
if not ex.cogagent_ready():
    print("CogAgent not loaded!"); sys.exit(1)
...
print(f"Screenshot: {w_raw}x{h_raw} -> {w}x{h} saved")
...
if m4:
    ...
    print(f"CLICK(4pts): [{x1},{y1},{x2},{y2}] -> ({cx},{cy}) on {w}x{h}")
elif m2:
    x1,y1 = int(m2.group(1)),int(m2.group(2))
    cx = int(x1 * w / 1000)
    cy = int(y1 * h / 1000)
    print(f"CLICK(2pts): [{x1},{y1}] -> ({cx},{cy}) on {w}x{h}")
else:
    print("No coordinates found."); sys.exit(0)

confirm = input("Execute click? (y/n): ").strip().lower()
if confirm == "y":
    # Scale back to real screen coordinates
    real_x = int(cx * w_raw / w)
    real_y = int(cy * h_raw / h)
    print(f"Clicking on real screen at ({real_x},{real_y})")
    ex._focus_ie()
    time.sleep(0.5)
    pyautogui.click(real_x, real_y)
    time.sleep(1)
    pyautogui.screenshot().save(SCRIPT_DIR / "screenshot_after.png")
    print("screenshot_after.png saved")
